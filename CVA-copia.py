...
#funcion para extraer la fecha del nombre de un archivo
def extraer_fecha(shapefile):
    """
    Extrae la fecha del nombre del shapefile en formato 'YYYY-MM-DD'.
    """
    # Unir las partes de la fecha para formar una cadena "YYYY-MM-DD"
    fecha_str = "-".join(shapefile.split("_")[1:4])  # Forma "2025-01-13"
...
def calcular_areas_shapefiles(directorio_salida, directorio_output_resources):
    ...
    with open(output_csv, 'wb') as csvfile:
        ...
        writer.writerow(['Shapefile', 'Total Area (m2)', 'Fecha', 'Cobertura en ha','Cobertura en km2', '% de cobertura', 'Cambio Ha', 'Cambio Cobertura', 'epoca'])
        ...
        #ordenar los shapefiles por fecha
        shapefiles.sort(key=extraer_fecha)

        # Inicializar variables previas para calcular los cambios
        prev_hectareas = prev_cobertura = None
        ...
        for shapefile in shapefiles:
            ...
            # fuera de la función
            _MESES_ESP = ['ene', 'feb', 'mar', 'abr', 'may', 'jun',
                          'jul', 'ago', 'sep', 'oct', 'nov', 'dic']

            fecha_obj = extraer_fecha(shapefile)  # <- tu función original que retorna datetime
            fecha_formateada = "{:02d}-{}-{:04d}".format(
                fecha_obj.day,
                _MESES_ESP[fecha_obj.month - 1],
                fecha_obj.year
            )
            ...
            #calculamos las hectareas y kilometros cuadrados
            ha = total_area / 10000.0
            km = ha/100
            #asignamos la epoca segun la fecha
            start_date = datetime.datetime(fecha_obj.year, 5, 20)  # 20 de mayo
            end_date = datetime.datetime(fecha_obj.year, 10, 15)  # 15 de octubre
            if start_date <= fecha_obj <= end_date:
                # Si la fecha está dentro del rango, es "lluvioso"
                print record[0]
                cobertura = ha / 11860.81  # Lluviosa
                epoca = "Lluviosa"
            else:
                # Si está fuera del rango, es "seca"
                cobertura = ha / 7018.81 # Seca
                epoca = "Seca"

            #calculo de cambio
            if prev_hectareas is None:
                cambio_hectareas = 0
            else:
                cambio_hectareas = ha - prev_hectareas

                # Cambio en cobertura (si es el primer valor, no hay cambio, se pone 0)
            if prev_cobertura is None:
                cambio_cobertura = 0
            else:
                cambio_cobertura = cobertura - prev_cobertura

            # Escribir el nombre del shapefile y el área total en el archivo CSV
            writer.writerow([shapefile, total_area, fecha_formateada, ha, km, cobertura, cambio_hectareas, cambio_cobertura, epoca] )


            # Actualizar las variables previas para la siguiente iteración
            prev_hectareas = ha
            prev_cobertura = cobertura
